...
The UI starts locked. Enter the database password in the page before any archived content is opened. On first use, enter the new password twice to create the encrypted key vault.
...
reddex uses two encrypted files with separate roles:

```text
data/reddex.db        # archived messages, encrypted by SQLCipher
data/reddex.keyvault  # random database key, encrypted by the user password
```

The terms are intentionally distinct:

- **database password**: the password you type in the CLI or web UI;
- **database key (`db_key`)**: a random 256-bit key generated by reddex;
- **key vault**: the SQLCipher-encrypted file that stores `db_key`.
...
reddex.keyvault
        ↓
random 256-bit db_key
        ↓
reddex.db
```

The password itself is not stored by reddex. After a successful unlock, `db_key` exists only in the running process memory. The main database uses SQLCipher raw-key syntax, so its random 256-bit key is used directly rather than being treated as another password.

This means stealing both `reddex.db` and `reddex.keyvault` is not sufficient to read the archive without the database password.

### First use

If neither file exists, reddex creates a new key vault and a new encrypted database.

- Web UI: enter the new password twice on the lock screen.
- CLI: `reddex init` or `reddex sync` asks for the new password twice.

An empty password is rejected.

### Failure is explicit

reddex deliberately does not contain a compatibility or migration layer for earlier database/key formats.

If a database exists but its matching `.keyvault` file does not exist, reddex fails immediately instead of generating a replacement key or trying to infer an old format.

Likewise, an incorrect password fails during key-vault unlock. reddex never falls back to plaintext SQLite.

There is no password-reset mechanism. Losing the database password makes the encrypted archive unrecoverable unless you have another valid copy of the key material.
...
`data/` contains the encrypted Reddit Chat database and its encrypted key vault. Do not commit or upload either file.

The database password is never written by reddex. The random database key is stored only inside the password-protected `reddex.keyvault` file and in process memory after unlock.
